...
@app.route('/catalog/<int:categories_id>/item/new',
           methods=['GET', 'POST'])
...
@app.route('/catalog/<int:categories_id>/Item/<int:items_id>/edit',
           methods=['GET', 'POST'])
...
@app.route('/catalog/<int:categories_id>/item/<int:items_id>/delete',
           methods=['get', 'post'])
def deleteCategoryItem(categories_id, items_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myFunction() {alert('You are not authorized "\
               "to delete items to this category. Please create your "\
